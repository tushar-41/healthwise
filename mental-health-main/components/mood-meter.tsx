...
type ScoreCategory = 'Positive' | 'Neutral' | 'Struggling';
        ...
        return (
          <motion.div key="step2" {...motionProps}>
            ...
            <p className="text-white/60 text-center mt-2 max-w-lg mx-auto">
              Think about how mentally and emotionally available you feel for challenges and responsibilities at this moment.
            </p>
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
          ...
          >
            <button type="button" onClick={onClose} className="absolute top-4 right-4 text-white/50 hover:text-white transition-colors" aria-label="Close modal"><X /></button>
...
const RecommendationCard = ({ icon: Icon, title, description, action, href, isPrimary = false }: Recommendation) => (
  <motion.a
    ...
    className={cn(
      "flex items-center gap-4 p-4 mt-2 bg-white/5 border border-white/10 rounded-lg text-left w-full",
